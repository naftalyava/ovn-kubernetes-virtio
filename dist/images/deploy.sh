./daemonset.sh --image-pull-policy=Always --image=docker.io/navadiaev/ovn-daemonset-f:latest --net-cidr=172.16.0.0/16 --svc-cidr=10.96.0.0/12 --gateway-mode="shared" --gateway-options="--gateway-interface=enp7s0" --ovnkube-node-mgmt-port-netdev=enp8s0 --k8s-apiserver=https://192.168.1.100:6443 --multicast-enabled --disable-snat-multiple-gws --disable-pkt-mtu-check=true
